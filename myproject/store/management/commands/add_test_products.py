...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if use_fixtures and os.path.exists(fixture_file):
            self.stdout.write(self.style.WARNING(f'Загрузка данных из фикстуры: {fixture_file}'))
            self.load_from_fixture(fixture_file)
        else:
            self.stdout.write(self.style.WARNING('Создание тестовых данных через ORM...'))
    ...
    def create_test_data(self):
        ...
        categories_data = [
            ...
            {"name": "Аксессуары", "description": "Чехлы, защитные стекла и другие аксессуары"},
    ...
    def load_from_fixture(self, fixture_file):
        ...
        try:
            ...
            self.stdout.write(self.style.SUCCESS(f"  Данные загружены из {fixture_file}"))
        except Exception as e:
            self.stdout.write(self.style.ERROR(f"  Ошибка загрузки фикстуры: {e}"))
            self.stdout.write(self.style.WARNING("  Создаю тестовые данные через ORM..."))
            self.create_test_data()
    ...
    def show_statistics(self):
        ...
        if products_count > 0:
            ...
            for product in Product.objects.all()[:5]:  # Первые 5 продуктов
                self.stdout.write(f"  - {product.name}: {product.price} ₽ ({product.category.name})")
